      ...
      <el-table :data="billList" style="width: 100%" border stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="payState" label="订单状态" width="80">
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.billRepay === true" style="color: #13ce66">退票取消</span>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="260" fixed="right">
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark" content="订单退票" placement="top" :enterable="false" :open-delay="500">
              <el-button type="danger" style="font-size: 18px;" @click="repayBillById(scope.row.billId)">
                 退票
              </el-button>
            </el-tooltip>
    ...
    <el-dialog title="修改订单" :visible.sync="editDialogVisible" width="60%" @close="editDialogClosed">
      <el-form :model="editForm" ref="editFormRef" label-width="100px">
        ...
        <el-form-item label="电影名称" prop="sysSession.sysMovie.movieName">
          <el-input v-model="editForm.sysSession.sysMovie.movieName" ></el-input>
        </el-form-item>
        <el-form-item label="影厅名称" prop="sysSession.sysHall.hallName">
          <el-input v-model="editForm.sysSession.sysHall.hallName" ></el-input>
        </el-form-item>
        <el-form-item label="座位" prop="sysSession.sysHall.hallName">
          <el-input v-model="editForm.seats" ></el-input>
        </el-form-item>
        <el-form-item label="场次日期" prop="sysSession.sessionDate">
          <el-input v-model="editForm.sysSession.sessionDate" ></el-input>
        </el-form-item>
        <el-form-item label="播放时间" prop="sysSession.sessionDate">
          <el-input v-model="editForm.sysSession.playTime" ></el-input>
        </el-form-item>
        <el-form-item label="散场时间" prop="sysSession.sessionDate">
          <el-input v-model="editForm.sysSession.endTime" ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    async repayBillById(id){
      const _this = this
      //询问售票员是否确认退票
      const resp = await this.$confirm('此操作将使本订单退票, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning',
        center: true,
        ...
        <div style="text-align:center;">
          <p>注意：电影开场后退票需要收取手续费</p>
          <p><input type="radio" name="payMethod" value="cash" id="cash"><label for="cash">全额退票</label></p>
          <p><input type="radio" name="payMethod" value="wechat" id="wechat"><label for="wechat">手续费退票</label></p>
        </div>`
      ...
      await axios.put('sysBill/repay/' + id).then(resp => {
        if (resp.data.code !== 200){
          _this.$message.success('退票失败！')
        }
      })
      this.getBillList()
